    ...
    def fit(
        ...
        *,
        ...
        Args:
            ...
            end_epoch: Ending epoch (exclusive). If None, uses self.epochs.
            action_epoch: Optional epoch at which to invoke on_action callback.
            on_action: Optional callback function invoked at action_epoch.
            deterministic: If True, disables shuffling for reproducibility.
            progress_bar: If True, displays a progress bar during training.
        """
        ...
        epoch_iter = range(start_epoch, end_epoch)
        if progress_bar:
            try:
                from tqdm import tqdm
                epoch_iter = tqdm(epoch_iter, desc="Training", unit="epoch")
            except ImportError:
                pass  # Fall back to regular range if tqdm not available
        ...
        for epoch in epoch_iter:
            ...
            for xb, yb in train_loader:
                ...
                loss.backward()
                if hasattr(self.model, 'capture_stats'):
            ...
            if progress_bar and hasattr(epoch_iter, 'set_postfix'):
                epoch_iter.set_postfix({
                    'train_loss': f'{cur_train_losses[-1]:.4f}',
